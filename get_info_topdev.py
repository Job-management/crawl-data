...
def get_company_name_topdev(source):
    return source.find('p', class_='my-1 line-clamp-1 text-base font-bold text-gray-500').get_text(' ', strip=True)

def get_title_topdev(source):
    return source.find('h1', class_='text-2xl font-bold text-black').get_text(' ', strip=True)
...
def get_description_topdev(source):
    div = source.find('div', class_='prose mb-4 max-w-full border-b border-gray-200 pb-2 text-sm last:mb-0 last:border-0 last:pb-0 lg:text-base')
...
def get_src_pic_topdev(source):
    div = source.find('div', class_ ='flex w-[21%] flex-initial items-center justify-center bg-white')
